...
conn = mysql.connect(host='localhost', user='root', password='123', database='bank')
rec = conn.cursor()
...
class BankApp:
    ...
    def create_main_menu(self):
        self.clear_window()
        tk.Label(self.root, text="____M A I N   M E N U____", font=("Arial", 30)).pack(pady=20)
        tk.Button(self.root, text="Bank Services", command=self.bank_services, width=30).pack(pady=5)
        tk.Button(self.root, text="Transaction", command=self.transaction, width=30).pack(pady=5)
        tk.Button(self.root, text="Reports", command=self.reports, width=30).pack(pady=5)
        tk.Button(self.root, text="Exit", command=self.exit_application, width=30).pack(pady=5)

    def bank_services(self):
        self.clear_window()
        tk.Label(self.root, text="B A N K   S E R V I C E S", font=("Arial", 30)).pack(pady=20)
        tk.Button(self.root, text="Create an Account", command=self.create_account, width=30).pack(pady=5)
        tk.Button(self.root, text="Display an Account", command=self.display_account, width=30).pack(pady=5)
        tk.Button(self.root, text="Delete an Account", command=self.delete_account, width=30).pack(pady=5)
        tk.Button(self.root, text="Return to Main Menu", command=self.create_main_menu, width=30).pack(pady=5)
